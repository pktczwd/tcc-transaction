...
public class FileSystemTransactionRepository extends CachableTransactionRepository {
    ...
    protected List<Transaction> doFindAllUnmodifiedSince(Date date) {
        ...
        for (Transaction transaction : allTransactions) {
            if (transaction.getTransactionType().equals(TransactionType.ROOT) && transaction.getLastUpdateTime().compareTo(date) < 0) {
    ...
    private void makeDirIfNecessory() {
        if (!initialized) synchronized (FileSystemTransactionRepository.class) {
            if (!initialized) {
